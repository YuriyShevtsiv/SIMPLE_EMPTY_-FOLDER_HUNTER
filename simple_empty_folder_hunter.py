...
# Описуємо головний клас програми, який є вікном застосунку.
class SimpleFolderHunter(ctk.CTk):
    # Короткий опис класу.
    """Проста версія Empty Folder Hunter для навчання."""

    # Системні папки, які ігноруємо при скануванні та видаленні.
    IGNORED_DIRS = {"$RECYCLE.BIN", "System Volume Information"}

    # Конструктор класу: запускається під час створення об'єкта.
    def __init__(self):
        # Викликаємо конструктор батьківського класу (вікна).
        super().__init__()

        # Встановлюємо назву вікна.
        self.title("Simple Empty Folder Hunter")
        # Встановлюємо стартовий розмір вікна.
        self.geometry("760x520")

        # Зберігаємо шлях, який вибере користувач.
        self.selected_path = ""
        # Зберігаємо знайдені порожні папки.
        self.empty_folders = []

        # Будуємо всі елементи інтерфейсу.
        self._build_ui()
    ...
    def clean_all(self):
        ...
        # Якщо користувач натиснув "Ні" — перериваємо операцію.
        if not ok:
            return
